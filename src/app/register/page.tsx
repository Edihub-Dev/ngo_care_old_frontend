...
export default function Register() {
  ...
  const validate = () => {
    ...
    if (!email.trim()) {
      nextErrors.email = 'Address is required';
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      alert('Register UI is ready. Connect API when you want.');
    } catch (error) {
  ...
  return (
    <div className={cn('min-h-screen', 'bg-zinc-50', 'flex', 'items-center', 'justify-center', 'px-4', 'py-4', 'relative')}>
      ...
      <Link href="/" className={cn('absolute', 'top-4', 'left-4', 'inline-flex', 'items-center', 'text-blue-600', 'hover:text-blue-700', 'text-sm')}>
